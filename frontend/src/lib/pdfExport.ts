...
function drawHeaderAndFooter(
  ...
  doc.setDrawColor('#84A59D'); // Sage accent
  doc.setLineWidth(0.4);
  doc.line(20, 18, 29, 9); // Hook handle diagonal
  // Hook Tip/Knob
  doc.setFillColor('#84A59D');
  doc.circle(29, 9, 0.5, 'F');

  // Draw Yarn Ball Circle
  doc.setFillColor('#F28482'); // Coral primary accent
  ...
  doc.setTextColor('#2D231B'); // Rich Charcoal/Brown
  doc.text('My Yarn Diary', 30, 13.2);

  // Brand Tagline
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(5.5);
  doc.setTextColor('#84A59D');
  doc.text('CRAFTER COMPANION', 30, 16.5);
  ...
  doc.setFontSize(7.5);
  doc.setTextColor('#A89F94');
  doc.text('My Yarn Diary • Handmade with Love', 20, 285.5);
...
export async function exportProjectToPdf(
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8);
  doc.setTextColor('#84A59D'); // Category label in accent color
  ...
  let statusColor = '#A89F94'; // OnHold / Planning default greyish
  if (project.status === ProjectStatus.InProgress) statusColor = '#F28482'; // Coral
  if (project.status === ProjectStatus.Completed) statusColor = '#84A59D'; // Sage
