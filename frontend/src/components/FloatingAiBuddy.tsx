...
export function FloatingAiBuddy({ onSelectTool, onDismiss }: FloatingAiBuddyProps) {
  ...
  return (
    <>
      {/* Fixed screen bounds container for smooth drag constraints & background z-index (z-25 sits under mobile backdrop z-30) */}
      <div ref={constraintsRef} className="fixed inset-4 pointer-events-none z-25">
        <div className="absolute bottom-safe right-2 pointer-events-none">
          <AnimatePresence>
            {isOpen && (
              <motion.div
                ...
                className="absolute bottom-20 right-0 bg-white border border-subtle rounded-2xl shadow-2xl p-4 w-72 pointer-events-auto flex flex-col gap-2 z-30"
              >
          ...
          <motion.div
            ...
            className="pointer-events-auto cursor-grab active:cursor-grabbing w-16 h-16 sewing-button flex items-center justify-center select-none shadow-2xl touch-none"
            onClick={() => {
