...
export function Dashboard({
 ...
 return <span className="text-[10px] bg-warning/10 text-warning border border-warning/20 px-2.5 py-1 rounded-full font-bold flex items-center gap-1 w-fit"><ClipboardList className="w-3 h-3 text-warning" /> Planning</span>;
 case ProjectStatus.InProgress:
 return <span className="text-[10px] bg-brand/10 text-brand border border-brand/20 px-2.5 py-1 rounded-full font-bold flex items-center gap-1 w-fit"><Route className="w-3 h-3 text-brand" /> In Progress</span>;
 case ProjectStatus.Completed:
 return <span className="text-[10px] bg-accent/10 text-accent border border-accent/20 px-2.5 py-1 rounded-full font-bold flex items-center gap-1 w-fit"><CircleCheckBig className="w-3 h-3 text-accent" /> Completed</span>;
 default:
 return <span className="text-[10px] bg-page text-muted border border-subtle px-2.5 py-1 rounded-full font-bold flex items-center gap-1 w-fit"><CirclePause className="w-3 h-3 text-muted" /> On Hold</span>;
 }
 };
 ...
 <div className="bg-white rounded-2xl p-3 border border-subtle warm-shadow flex items-center justify-between animate-fade-in">
 <div>
 <span className="text-[9px] uppercase font-semibold tracking-wider text-muted block font-mono leading-tight">In Progress</span>
 <span className="text-xl font-bold font-serif text-heading mt-1 block">{totalWips}</span>
 </div>
 <span className="bg-brand/10 border border-brand/20 w-8 h-8 rounded-lg flex items-center justify-center"><Route className="w-4 h-4 text-brand" /></span>
 </div>

 <div className="bg-white rounded-2xl p-3 border border-subtle warm-shadow flex items-center justify-between animate-fade-in animate-duration-75">
 <div>
 <span className="text-[9px] uppercase font-semibold tracking-wider text-muted block font-mono leading-tight">Completed</span>
 <span className="text-xl font-bold font-serif text-heading mt-1 block">{completedCount}</span>
 </div>
 <span className="bg-accent/10 border border-accent/20 w-8 h-8 rounded-lg flex items-center justify-center"><CircleCheckBig className="w-4 h-4 text-accent" /></span>
 </div>

 <div className="bg-white rounded-2xl p-3 border border-subtle warm-shadow flex items-center justify-between animate-fade-in animate-duration-100">
 <div>
 <span className="text-[9px] uppercase font-semibold tracking-wider text-muted block font-mono leading-tight">Planning</span>
 <span className="text-xl font-bold font-serif text-heading mt-1 block">{planningCount}</span>
 </div>
 <span className="bg-warning/10 border border-warning/20 w-8 h-8 rounded-lg flex items-center justify-center"><ClipboardList className="w-4 h-4 text-warning" /></span>
 </div>

 <div className="bg-white rounded-2xl p-3 border border-subtle warm-shadow flex items-center justify-between animate-fade-in animate-duration-150">
 <div>
 <span className="text-[9px] uppercase font-semibold tracking-wider text-muted block font-mono leading-tight">On Hold</span>
 <span className="text-xl font-bold font-serif text-heading mt-1 block">{onHoldCount}</span>
 </div>
 <span className="bg-page border border-subtle w-8 h-8 rounded-lg flex items-center justify-center"><CirclePause className="w-4 h-4 text-muted" /></span>
 </div>

 <div className="bg-white rounded-2xl p-3 border border-subtle warm-shadow flex items-center justify-between animate-fade-in animate-duration-200">
 <div>
 <span className="text-[9px] uppercase font-semibold tracking-wider text-muted block font-mono leading-tight">Total Rows</span>
 <span className="text-xl font-bold font-serif text-heading mt-1 block">{totalStitches}</span>
 </div>
 ...
 <div className="bg-white rounded-2xl p-3 border border-subtle warm-shadow flex items-center justify-between animate-fade-in animate-duration-300">
 <div>
 <span className="text-[9px] uppercase font-semibold tracking-wider text-muted block font-mono leading-tight">Success Rate</span>
 <span className="text-xl font-bold font-serif text-heading mt-1 block">{completionRate}%</span>
 </div>
 <span className="bg-amber-50 border border-amber-100 w-8 h-8 rounded-lg flex items-center justify-center"><Trophy className="w-4 h-4 text-amber-500" /></span>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-2 sm:gap-4 border-b border-subtle pb-2.5 sm:pb-4 bg-white p-2.5 sm:p-5 rounded-2xl sm:rounded-3xl mt-2 sm:mt-4 warm-shadow animate-fade-in">
        <div className="flex items-center gap-1.5 sm:gap-4 w-full lg:w-auto overflow-x-auto sm:overflow-visible pb-1 sm:pb-0">
          {isSidebarCollapsed ? (
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            {/* Stage Filter */}
            <div className="relative min-w-[95px] sm:min-w-[140px]">
              <CustomDropdown
                value={stageFilter}
                onChange={(val) => {
            ...
            {/* Sort selector */}
            <div className="relative min-w-[115px] sm:min-w-[200px]">
              <CustomDropdown
                value={sortBy}
        ...
        <div className="flex items-center gap-2 w-full lg:w-auto mt-1 lg:mt-0">
          ...
          {!isFavouritesCategory && categoryId !== 'archived' && (
            <button
              ...
              className="sewing-button px-3 sm:px-8 py-1.5 sm:py-2 flex items-center justify-center text-xs sm:text-sm whitespace-nowrap shrink-0"
            >
              ...
              <div className="p-3.5 sm:p-6 space-y-2.5 sm:space-y-4">
                <div className="flex justify-between items-start gap-2 sm:gap-4">
                  <div className="space-y-0.5 sm:space-y-1 min-w-0 flex-1">
                    <span className="text-[8px] sm:text-[9px] font-mono font-bold text-muted block flex items-center gap-1 uppercase tracking-widest">
                ...
                {getProjectCoverPhoto(p) ? (
                  <div className="w-full h-24 sm:h-32 rounded-xl sm:rounded-2xl border border-subtle overflow-hidden bg-page relative">
                    <img
                  ...
                  <div className="w-full h-24 sm:h-32 rounded-xl sm:rounded-2xl border border-subtle bg-surface flex flex-col items-center justify-center text-muted gap-1 shrink-0">
                    <span className="text-xl sm:text-2xl">🧶</span>
                    <span className="text-[9px] sm:text-[10px] font-bold uppercase tracking-wider">No photos yet</span>
                  </div>
                )}

                {p.notes && (
                  <p className="text-[10px] sm:text-[11px] text-muted italic max-w-full truncate font-semibold">"{p.notes}"</p>
                )}
              </div>
 ...
 <span>Row Count Mark: <span className="text-brand font-mono">{p.rowCount}</span></span>
 </div>
 ...
 className="p-1 px-1.5 bg-white hover:bg-red-55 text-muted hover:text-red-500 border border-subtle hover:border-red-100 rounded-lg shrink-0 cursor-pointer"
 title="Delete project"
 >
 <Trash2 className="w-3.5 h-3.5 cursor-pointer" />
 </button>
 <span className="text-[10px] font-extrabold text-accent group-hover:translate-x-1 duration-150 flex items-center gap-0.5">
 Open Project
 ...
 <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1">
              <label className="text-[10px] font-extrabold text-muted uppercase tracking-wider block">Journal Category</label>
              <CustomDropdown
            ...
            <div className="space-y-1">
 <label className="text-[10px] font-extrabold text-muted uppercase tracking-wider block">Project Title *</label>
 <input
 type="text"
 required
 disabled={isSubmitting}
